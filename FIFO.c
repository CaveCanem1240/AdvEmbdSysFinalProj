...
jsDataType volatile *CursorPutPt; // put next
jsDataType volatile *CursorGetPt; // get next
jsDataType static CursorFifo[JSFIFOSIZE];
Sema4Type CursorFifoAvailable;

// initialize pointer FIFO
void CursorFifo_Init(void){ long sr;
  sr = StartCritical();      // make atomic
	OS_InitSemaphore(&CursorFifoAvailable, 0);
  CursorPutPt = CursorGetPt = &CursorFifo[0]; // Empty
  EndCritical(sr);
}

// add element to end of pointer FIFO
// return RXFIFOSUCCESS if successful
int CursorFifo_Put(jsDataType data){
  jsDataType volatile *nextPutPt;
  nextPutPt = CursorPutPt+1;
  if(nextPutPt == &CursorFifo[JSFIFOSIZE]){
    nextPutPt = &CursorFifo[0];  // wrap
  }
  if(nextPutPt == CursorGetPt){
    return(JSFIFOFAIL);      // Failed, fifo full
  }
  else{
    *(CursorPutPt) = data;       // Put
    CursorPutPt = nextPutPt;     // Success, update
		OS_Signal(&CursorFifoAvailable);
...
// return RXFIFOSUCCESS if successful
int CursorFifo_Get(jsDataType *datapt){
  OS_Wait(&CursorFifoAvailable);
  *datapt = *(CursorGetPt++);
  if(CursorGetPt == &CursorFifo[JSFIFOSIZE]){
     CursorGetPt = &CursorFifo[0];   // wrap
  }
  return(JSFIFOSUCCESS);
}
