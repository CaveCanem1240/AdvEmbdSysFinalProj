...
//  PWM0A_Init(40000, 10000);         // initialize PWM0, 1000 Hz, 75% duty
//  PWM0B_Init(40000, 30000);         // initialize PWM0, 1000 Hz, 25% duty
//	PWM6C_Init(40000, 30000);
	PWM6C_Init(40000, 10000);
	uint16_t i;
	i = 0;
	while(i<10000){
    i++;
...
//			color = color+counter;
		...
		BSP_LCD_DrawBitmap(0, 127-64*DisplayDevice, Map_Image, IMGW, IMGH);
		counter+=10;
		OS_Suspend();
	}
}
...
int Device_Init(int16_t x, int16_t y){
	BSP_LCD_OutputInit();
	BSP_LCD_FillScreen(BGCOLOR);
//	BSP_LCD_DrawBitmap(x, y, pt_img, 64, 64);
	return 1;
}



int Testmain9(void){       // Testmain9
	
  OS_Init();
	StackDataType StackData;
	StackData.func = &Device_Init;
	if(StackData.func == &Device_Init){
		Device_Init(31,127);
	}
	OS_Launch(TIME_1MS); // doesn't return, interrupts enabled in here
  return 0;            // this never executes
