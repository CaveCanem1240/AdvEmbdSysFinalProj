...
// ******** OS_WaitC ************
// Quit and return 0 if condition doesn't match the value
// decrement semaphore 
// input:  pointer to a counting semaphore, pointer to a condition, condition value
// output: 0 if quit by condition mismatching, 1 if successfully decrement semaphore
uint8_t OS_WaitC(Sema4Type *semaPt, uint32_t *Dstate, uint32_t value){
	OS_DisableInterrupts();
	while (semaPt->Value == 0 && *Dstate == value){
		OS_EnableInterrupts();
		OS_Suspend();
		OS_DisableInterrupts();
	}
...
// input:  pointer to a counting semaphore
// output: none
void OS_SignalC(Sema4Type *semaPt){
	OS_DisableInterrupts();
	semaPt->Value += 1;
	OS_EnableInterrupts();
	OS_DisableInterrupts();
